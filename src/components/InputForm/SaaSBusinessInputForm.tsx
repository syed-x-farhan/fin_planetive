...
interface SaaSBusinessInputFormProps {
...
export const SaaSBusinessInputForm: React.FC<SaaSBusinessInputFormProps> = ({ onSubmit, onBack, initialValues }) => {
  // Section toggles
  const [hasPlans, setHasPlans] = useState(initialValues?.plans ? true : true);
  const [hasExpenses, setHasExpenses] = useState(initialValues?.expenses ? true : false);
  ...
  // Dynamic lists
  const [plans, setPlans] = useState<SaaSPlan[]>(initialValues?.plans || [{ name: '', price: '', users: '', growth: '', churn: '', costPerUser: '' }]);
  const [expenses, setExpenses] = useState<CostItem[]>(initialValues?.expenses || []);
  ...
  // Forecasting
  const [forecastPeriod, setForecastPeriod] = useState(initialValues?.forecast?.period || '12'); // months default
  ...
  const handlePlanChange = (idx: number, field: keyof SaaSPlan, value: string) => {
    setPlans(s => s.map((item, i) => i === idx ? { ...item, [field]: value } : item));
  };
  const addPlan = () => setPlans(s => [...s, { name: '', price: '', users: '', growth: '', churn: '', costPerUser: '' }]);
  const removePlan = (idx: number) => setPlans(s => s.filter((_, i) => i !== idx));

  const handleExpenseChange = (idx: number, field: keyof CostItem, value: string) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Convert annual costs to monthly if needed
    let processedExpenses = expenses;
    if (expenseInputType === 'annual') {
      processedExpenses = expenses.map(expense => ({
        ...expense,
        amount: expense.amount ? (parseFloat(expense.amount) / 12).toString() : ''
      }));
    }
    // Convert annual revenue to monthly if needed
    let processedPlans = plans;
    if (revenueInputType === 'annual') {
      processedPlans = plans.map(plan => ({
        ...plan,
        users: plan.users ? (parseFloat(plan.users) / 12).toString() : '',
      }));
    }
    // Filter out plans with missing or invalid required fields
    const filteredPlans = (revenueInputType === 'annual' ? processedPlans : plans).filter(p => {
      const priceValid = p.price !== '' && !isNaN(Number(p.price));
      const usersValid = p.users !== '' && !isNaN(Number(p.users));
      const costPerUserValid = p.costPerUser !== '' && !isNaN(Number(p.costPerUser));
      return priceValid && usersValid && costPerUserValid;
    });
    if (hasPlans && filteredPlans.length === 0) {
      alert("Please add at least one SaaS plan with valid price, users, and cost per user.");
      return;
    }
    onSubmit({
      plans: hasPlans ? filteredPlans.map(p => ({
        ...p,
        price: sanitizeNumber(p.price),
        users: sanitizeNumber(p.users),
        growth: sanitizeNumber(p.growth),
        costPerUser: sanitizeNumber(p.costPerUser),
      })) : [],
      ...
      accountsPayable: hasAP ? { days: sanitizeNumber(apDays) } : undefined,
      ...
      forecast: { period: sanitizeNumber(forecastPeriod), type: forecastType },
  ...
  React.useEffect(() => {
    if (initialValues) {
      setPlans(initialValues.plans || [{ name: '', price: '', users: '', growth: '', churn: '', costPerUser: '' }]);
      ...
      setHasPlans(initialValues.plans ? true : true);
      setHasExpenses(initialValues.expenses ? true : false);
    ...
    <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-10 py-8 px-2">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            SaaS Plans / Products
            <Switch checked={hasPlans} onCheckedChange={setHasPlans} />
          </CardTitle>
          <div className="mt-1 mb-2 flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
            <div className="font-semibold text-base">What SaaS products or plans do you offer?</div>
              <div className="text-sm text-muted-foreground">List each plan, its price, user count per {revenueInputType === 'monthly' ? 'month' : 'year'}, growth, churn, and cost per user.</div>
            </div>
        ...
        {hasPlans && (
          <CardContent className="space-y-2 mb-10">
            <div className="font-semibold text-lg mb-4">Plans</div>
            <Label className="block mb-4">Add your SaaS plans or products</Label>
            {/* Header Row */}
            <div className="grid grid-cols-7 gap-4 items-center mb-2 px-1">
              <div className="text-xs font-semibold text-muted-foreground">Plan Name</div>
              <div className="text-xs font-semibold text-muted-foreground">Price/User</div>
              <div className="text-xs font-semibold text-muted-foreground">Users/{revenueInputType === 'monthly' ? 'Month' : 'Year'}</div>
              <div className="text-xs font-semibold text-muted-foreground">Growth %</div>
              <div className="text-xs font-semibold text-muted-foreground">Churn %</div>
              <div className="text-xs font-semibold text-muted-foreground">Cost/User</div>
              <div className="text-xs" />
            </div>
            {/* Dynamic Rows */}
            {plans.map((plan, idx) => (
              <div key={idx} className="grid grid-cols-7 gap-4 items-center mb-2 px-1">
                <Input placeholder="Plan Name" value={plan.name} onChange={e => handlePlanChange(idx, 'name', e.target.value)} />
                <Input placeholder="$20" value={plan.price} onChange={e => handlePlanChange(idx, 'price', e.target.value)} />
                <Input placeholder={revenueInputType === 'monthly' ? '100' : '1200'} value={plan.users} onChange={e => handlePlanChange(idx, 'users', e.target.value)} />
                <Input placeholder="10%" value={plan.growth} onChange={e => handlePlanChange(idx, 'growth', e.target.value)} />
                <Input placeholder="5%" value={plan.churn} onChange={e => handlePlanChange(idx, 'churn', e.target.value)} />
                <Input placeholder="$5" value={plan.costPerUser} onChange={e => handlePlanChange(idx, 'costPerUser', e.target.value)} />
                <Button type="button" variant="ghost" size="icon" onClick={() => removePlan(idx)} className="justify-self-end"><Trash2 className="w-4 h-4" /></Button>
              </div>
            ))}
            <Button type="button" variant="outline" onClick={addPlan} className="mt-2"><Plus className="w-4 h-4 mr-1" /> Add Another Plan</Button>
          </CardContent>
        )}
      </Card>

      {/* Business Regular Costs Section */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
          ...
          <div className="mt-1 mb-2 flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <div className="text-sm text-muted-foreground">Add your recurring costs like hosting, salaries, marketing, or support.</div>
            </div>
            <div className="mt-2 md:mt-0 flex items-center gap-2">
        ...
        {hasExpenses && (
          <CardContent className="space-y-2 mb-10">
            <div className="font-semibold text-lg mb-4">{expenseInputType === 'monthly' ? 'Monthly Costs' : 'Annual Costs'}</div>
            <Label className="block mb-4">Add your {expenseInputType === 'monthly' ? 'monthly' : 'annual'} costs</Label>
            {/* Header Row for Costs */}
            <div className="flex gap-x-4 items-center mb-2 text-xs font-medium text-muted-foreground">
              <div className="w-40">Category</div>
              <div className="w-24">Amount ({expenseInputType === 'monthly' ? '$/month' : '$/year'})</div>
            ...
            {expenses.map((expense, idx) => (
              <div key={idx} className="flex gap-x-4 items-center mb-2 px-1">
                <Input list="cost-categories" placeholder="Category (e.g. Hosting)" value={expense.category} onChange={e => handleExpenseChange(idx, 'category', e.target.value)} className="w-40" />
                <datalist id="cost-categories">
                  {COMMON_COSTS.map((cat) => <option key={cat} value={cat} />)}
                </datalist>
            ...
            <Button type="button" variant="teal" onClick={addExpense} className="mt-2"><Plus className="w-4 h-4 mr-1" /> Add Another Cost</Button>
          </CardContent>
        )}
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="mt-1 mb-2">
            ...
            <div className="text-sm text-muted-foreground">List any one-time purchases like laptops, servers, or furniture for your SaaS business.</div>
          </div>
        </CardHeader>
        {hasEquipment && (
          <CardContent className="space-y-2 mb-10">
            <div className="font-semibold text-lg mb-4">Equipment</div>
            <Label className="block mb-4">Have you bought any equipment or tools for your SaaS?</Label>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="mt-1 mb-2">
            <div className="font-semibold text-base">How did you start your SaaS?</div>
      ...
      <Card>
        ...
        {hasOther && (
          <CardContent className="space-y-2 mb-10">
            ...
            <div className="flex gap-x-4 items-center mb-2 text-xs font-medium text-muted-foreground">
              <div className="w-40">Type/Description</div>
              <div className="w-24">Amount</div>
              <div className="w-24">Income?</div>
              <div className="w-40">Notes</div>
              <div className="w-8" />
            </div>
            {other.map((item, idx) => (
              <div key={idx} className="flex gap-x-4 items-center mb-2 px-1">
                <Input placeholder="Type (e.g. Grant)" value={item.type} onChange={e => handleOtherChange(idx, 'type', e.target.value)} className="w-40" />
